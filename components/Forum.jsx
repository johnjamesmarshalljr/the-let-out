...
import { ChevronUp, ChevronDown, MessageCircle, Plus, Home, X, LogOut, Camera, Pencil } from "lucide-react";
...
/* ---------- avatar (photo if set, else colored initial) ---------- */
function Avatar({ name, url, color, size = 32 }) {
  if (url) {
    return (
      <img
        src={url}
        alt={name || ""}
        style={{ width: size, height: size, borderRadius: size, objectFit: "cover", flexShrink: 0, background: C.panel2 }}
      />
    );
  }
  ...
  return (
    <div style={{
      ...
      background: color || `linear-gradient(135deg, ${C.gold}, ${C.violet})`,
...
function Vote({ score, vote, onUp, onDown }) {
  ...
  const btn = (active, bg, border, color, idle) => ({
    display: "flex", alignItems: "center", justifyContent: "center",
    width: 30, height: 28, borderRadius: 8,
    border: `1px solid ${active ? border : C.border}`,
    background: active ? bg : "transparent",
    color: active ? color : idle, cursor: "pointer", transition: "all .12s",
  });
  return (
...
/* ---------- profile form (used for onboarding AND editing) ---------- */
function ProfileForm({ mode, me, initial, onSaved, onCancel }) {
  ...
  const pickFile = async (e) => {
    ...
    if (error) {
      setErr("Photo upload failed: " + error.message + " — check that the 'avatars' storage bucket exists.");
  ...
  const save = async () => {
    ...
    if (error) {
      if (error.code === "23505") setErr("That username is taken — try another.");
      else setErr("Could not save: " + error.message);
      setSaving(false); return;
    }
    setSaving(false);
    onSaved({ ...initial, ...payload, id: me.id });
  };

  return (
    <div style={{ maxWidth: 520 }}>
      {mode === "edit" && (
        <button onClick={onCancel} style={{ fontWeight: 600, marginBottom: 16, color: C.muted, fontSize: 13, background: "none", border: "none", cursor: "pointer", padding: 0 }}>← cancel</button>
      )}
      ...
      <p style={{ color: C.muted, fontSize: 14, margin: "0 0 22px" }}>
        {mode === "edit" ? "Update how you show up on the Let Out." : "This is how the scene sees you here. Pick a name — the rest is optional and you can change it anytime."}
      </p>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 22 }}>
        ...
        <div>
          ...
          <button onClick={() => fileRef.current && fileRef.current.click()} disabled={uploading}
            style={{ display: "flex", alignItems: "center", gap: 7, fontWeight: 700, fontSize: 13, background: C.panel2, color: C.text, border: `1px solid ${C.border}`, borderRadius: 999, padding: "8px 14px", cursor: "pointer" }}>
            <Camera size={15} /> {uploading ? "Uploading…" : avatarUrl ? "Change photo" : "Upload photo"}
          </button>
          {avatarUrl && (
            <button onClick={() => setAvatarUrl(null)} style={{ display: "block", marginTop: 8, color: C.mutedDim, fontSize: 12, background: "none", border: "none", cursor: "pointer", padding: 0 }}>
              remove photo
            </button>
          )}
        </div>
      ...
      {/* color (only matters with no photo) */}
      {!avatarUrl && (
          ...
          <div style={{ display: "flex", gap: 10 }}>
            {AVATAR_COLORS.map((col) => (
              <button key={col} onClick={() => setAvatarColor(col)} style={{
                width: 30, height: 30, borderRadius: 30, background: col, cursor: "pointer",
                border: avatarColor === col ? `3px solid ${C.text}` : `2px solid ${C.border}`,
              }} />
            ))}
          </div>
        </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 18, flexWrap: "wrap" }}>
        {[["kiki", "Kiki"], ["mainstream", "Mainstream"], ["both", "Both"]].map(([v, l]) => (
          <button key={v} onClick={() => setScene(scene === v ? null : v)} style={{
            fontWeight: 700, fontSize: 13, padding: "8px 16px", borderRadius: 999, cursor: "pointer",
            border: `1px solid ${scene === v ? C.magenta : C.border}`,
            background: scene === v ? C.magenta : "transparent",
            color: scene === v ? C.ink : C.muted,
          }}>{l}</button>
        ))}
      </div>
      ...
      <button onClick={save} disabled={saving || uploading}
        style={{ fontWeight: 700, background: `linear-gradient(135deg, ${C.magenta}, ${C.violet})`, color: C.ink, borderRadius: 999, padding: "11px 26px", fontSize: 14, border: "none", cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1 }}>
        {saving ? "Saving…" : mode === "edit" ? "Save changes" : "Enter the Let Out"}
      </button>
    </div>
  );
}
...
export default function Forum() {
  ...
  const [me, setMe] = useState(null);          // full profile + {email, suggestion}
  ...
  const hydrateUser = useCallback(async (user) => {
    ...
    if (!prof) {
      await supabase.from("profiles").insert({ id: user.id, onboarded: false });
      prof = { id: user.id, onboarded: false };
    }
    const suggestion = (user.user_metadata && (user.user_metadata.full_name || user.user_metadata.name)) || (user.email ? user.email.split("@")[0] : "");
    const cleanSuggestion = suggestion.toLowerCase().replace(/[^a-z0-9_.]/g, "").slice(0, 20);
    setMe({ ...prof, email: user.email, suggestion: cleanSuggestion });
  ...
  /* voting — optimistic, reverts and shows error if the DB write fails */
  const applyVote = async (postId, dir) => {
    ...
    let error;
    if (next === null) {
      ({ error } = await supabase.from("votes").delete().eq("post_id", postId).eq("user_id", me.id));
    } else {
      ({ error } = await supabase.from("votes").upsert(
    ...
    if (error) {
      ...
      setVoteError("Vote didn't save: " + error.message);
  ...
  const openPost = async (id) => {
    setSelectedId(id); setView("post"); setCommentText(""); setComments([]);
    const { data } = await supabase.from("comment_feed").select("*").eq("post_id", id).order("created_at", { ascending: true });
    setComments(data || []);
  };
  ...
  const submitComment = async () => {
    if (!requireIdentity() || !commentText.trim() || !selectedId || busy) return;
    setBusy(true);
    const { error } = await supabase.from("comments").insert({ post_id: selectedId, author_id: me.id, body: commentText.trim() });
    if (!error) {
      setCommentText("");
      const { data } = await supabase.from("comment_feed").select("*").eq("post_id", selectedId).order("created_at", { ascending: true });
      setComments(data || []);
    ...
    <div style={{ minHeight: "100vh", background: C.ink, color: C.text }}>
      <header style={{ position: "sticky", top: 0, zIndex: 20, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", height: 60, borderBottom: `1px solid ${C.border}`, background: "rgba(20,16,31,0.85)", backdropFilter: "blur(10px)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <button onClick={() => { if (requireIdentity()) setView("create"); }} style={{ ...pill(`linear-gradient(135deg, ${C.magenta}, ${C.violet})`, C.ink), display: "flex", alignItems: "center", gap: 6 }}>
            <Plus size={16} strokeWidth={2.6} /> Post
          </button>
          {me ? (
            <>
              <button onClick={() => me.username ? openProfile(me.username) : setView("onboarding")} style={{ background: "none", border: "none", cursor: "pointer", padding: 0 }}>
                <Avatar name={me.username} url={me.avatar_url} color={me.avatar_color} />
              </button>
      ...
      <div style={{ display: "flex", maxWidth: 1000, margin: "0 auto" }}>
        <aside className="rail" style={{ flexShrink: 0, padding: "20px 12px", width: 200, borderRight: `1px solid ${C.border}` }}>
          <button onClick={() => navTo("home")} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", fontWeight: 700, marginBottom: 12, padding: "9px 10px", borderRadius: 9, fontSize: 14, border: "none", cursor: "pointer", color: room === "home" ? C.ink : C.text, background: room === "home" ? C.gold : "transparent" }}>
            <Home size={16} /> Home
          </button>
          ...
          {ROOMS.map((r) => (
            <button key={r} onClick={() => navTo(r)} style={{ display: "block", width: "100%", textAlign: "left", fontWeight: 600, padding: "8px 10px", borderRadius: 9, fontSize: 14, border: "none", cursor: "pointer", background: room === r ? C.panel2 : "transparent", color: room === r ? C.text : C.muted }}>{r}</button>
          ))}
        </aside>
        ...
        <main style={{ flex: 1, minWidth: 0, padding: "20px 24px" }}>
          {loading ? (
            ...
            <ProfileForm mode="onboard" me={me} initial={{ username: me.username || me.suggestion }} onSaved={onProfileSaved} />
          ) : view === "edit" && me ? (
            <ProfileForm mode="edit" me={me} initial={me} onSaved={(u) => { onProfileSaved(u); openProfile(u.username); }} onCancel={() => me.username && openProfile(me.username)} />
          ) : (
            <>
              ...
              {view === "post" && selected && (
                <PostDetail post={selected} comments={comments} vote={votes[selected.id]} applyVote={applyVote} back={() => setView("feed")} openProfile={openProfile} me={me} commentText={commentText} setCommentText={setCommentText} submitComment={submitComment} promptSignIn={() => setShowSignIn(true)} goOnboard={() => setView("onboarding")} inputStyle={inputStyle} busy={busy} />
              )}
              {view === "profile" && profileData && (
                <Profile profile={profileData} posts={posts} openPost={openPost} back={() => setView("feed")} isMe={!!(me && me.username && me.username === profileData.username)} onEdit={() => setView("edit")} />
              )}
              {view === "create" && me && <Create draft={draft} setDraft={setDraft} submitPost={submitPost} back={() => setView("feed")} inputStyle={inputStyle} busy={busy} />}
            </>
          )}
        </main>
      </div>
      ...
      {voteError && (
        <div style={{ position: "fixed", bottom: 20, left: "50%", transform: "translateX(-50%)", zIndex: 40, background: C.panel, border: `1px solid ${C.magenta}`, color: C.text, borderRadius: 10, padding: "10px 16px", fontSize: 13, maxWidth: "90%" }}>{voteError}</div>
      )}

      {showSignIn && (
        <div style={{ position: "fixed", inset: 0, zIndex: 30, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, background: "rgba(8,6,14,0.7)" }} onClick={() => { setShowSignIn(false); setLinkSent(false); }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: C.panel, border: `1px solid ${C.borderHot}`, borderRadius: 18, padding: 24, width: 360, maxWidth: "100%" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
              ...
              <button onClick={() => { setShowSignIn(false); setLinkSent(false); }} style={{ color: C.muted, background: "none", border: "none", cursor: "pointer" }}><X size={18} /></button>
            </div>
            {linkSent ? (
              ...
              <>
                <p style={{ color: C.muted, fontSize: 13, marginBottom: 18 }}>Sign in, pick a name, and you're walking. Takes about ten seconds.</p>
                <button onClick={signInGoogle} style={{ width: "100%", fontWeight: 700, marginBottom: 14, background: "#fff", color: "#1a1a1a", borderRadius: 10, padding: 11, border: "none", cursor: "pointer", fontSize: 14 }}>Continue with Google</button>
                ...
                <button onClick={sendMagicLink} style={{ width: "100%", fontWeight: 700, background: `linear-gradient(135deg, ${C.magenta}, ${C.violet})`, color: C.ink, borderRadius: 10, padding: 11, border: "none", cursor: "pointer", fontSize: 14 }}>Email me a sign-in link</button>
              </>
            )}
          </div>
        </div>
...
/* ---------- subviews ---------- */
function Byline({ author, house, avatar, color, size, openProfile, muted }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 12.5, color: C.muted }}>
      ...
      <button onClick={() => author && openProfile(author)} style={{ fontWeight: 700, color: muted ? C.muted : C.text, background: "none", border: "none", cursor: "pointer", padding: 0 }}>{author || "unknown"}</button>
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 4, background: C.panel, borderRadius: 999, padding: 3 }}>
        {["hot", "new", "top"].map((s) => (
          <button key={s} onClick={() => setSort(s)} style={{ textTransform: "uppercase", fontWeight: 700, fontSize: 11, letterSpacing: "0.1em", padding: "6px 14px", borderRadius: 999, border: "none", cursor: "pointer", color: sort === s ? C.ink : C.muted, background: sort === s ? C.gold : "transparent" }}>{s}</button>
        ))}
      </div>
    </div>
  );
    ...
    <div style={{ display: "flex", gap: 12, background: C.panel, border: `1px solid ${C.border}`, borderRadius: 14, padding: 14, marginBottom: 12 }}>
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7, flexWrap: "wrap" }}>
          ...
          <Byline author={post.author} house={post.author_house} avatar={post.author_avatar} color={post.author_color} size={20} openProfile={openProfile} muted />
...
function Feed({ visible, room, sort, setSort, votes, applyVote, openPost, openProfile }) {
  return (
    <div>
      ...
      {visible.length === 0 ? (
        <div style={{ border: `1px dashed ${C.border}`, borderRadius: 14, padding: 32, textAlign: "center", color: C.muted }}>Nothing here yet. Be the first to post.</div>
      ) : visible.map((p) => (
...
function PostDetail({ post, comments, vote, applyVote, back, openProfile, me, commentText, setCommentText, submitComment, promptSignIn, goOnboard, inputStyle, busy }) {
  const canReply = me && me.onboarded;
  const replyAction = !me ? promptSignIn : !me.onboarded ? goOnboard : submitComment;
  const replyLabel = !me ? "Sign in to reply" : !me.onboarded ? "Finish your profile to reply" : "Reply";
  return (
    <div>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
              <span style={{ fontWeight: 700, color: C.violet, fontSize: 12.5 }}>{post.category}</span>
              <span style={{ color: C.mutedDim }}>·</span>
              <Byline author={post.author} house={post.author_house} avatar={post.author_avatar} color={post.author_color} size={22} openProfile={openProfile} muted />
              <span style={{ color: C.mutedDim, fontSize: 12.5 }}>· {timeAgo(post.created_at)}</span>
            </div>
      ...
      <div style={{ marginBottom: 18 }}>
        <textarea value={commentText} onChange={(e) => setCommentText(e.target.value)} placeholder={canReply ? "Add your reply…" : replyLabel} disabled={!canReply} rows={3} style={{ ...inputStyle, resize: "vertical", opacity: canReply ? 1 : 0.6 }} />
        <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 8 }}>
          <button onClick={replyAction} disabled={busy} style={{ fontWeight: 700, background: `linear-gradient(135deg, ${C.magenta}, ${C.violet})`, color: C.ink, borderRadius: 999, padding: "8px 18px", fontSize: 13, border: "none", cursor: "pointer", opacity: busy ? 0.6 : 1 }}>{replyLabel}</button>
        </div>
      </div>

      {comments.map((c) => (
        <div key={c.id} style={{ display: "flex", gap: 12, padding: "12px 0", borderTop: `1px solid ${C.border}` }}>
          <Avatar name={c.author} url={c.author_avatar} color={c.author_color} size={32} />
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 4, fontSize: 12 }}>
              <button onClick={() => c.author && openProfile(c.author)} style={{ fontWeight: 700, color: C.text, background: "none", border: "none", cursor: "pointer", padding: 0 }}>{c.author || "unknown"}</button>
              {c.author_house ? <span style={{ color: C.violet, fontWeight: 600 }}>· {c.author_house}</span> : null}
    ...
    <div>
      ...
      <div style={{ background: `linear-gradient(135deg, ${C.panel2}, ${C.panel})`, border: `1px solid ${C.borderHot}`, borderRadius: 16, padding: 22 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          {isMe && (
            <button onClick={onEdit} style={{ display: "flex", alignItems: "center", gap: 6, fontWeight: 700, fontSize: 13, background: C.panel2, color: C.text, border: `1px solid ${C.border}`, borderRadius: 999, padding: "8px 14px", cursor: "pointer", alignSelf: "flex-start" }}><Pencil size={14} /> Edit</button>
          )}
        </div>
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      ...
      <select value={draft.room} onChange={(e) => setDraft({ ...draft, room: e.target.value })} style={{ ...inputStyle, marginBottom: 16 }}>
        {ROOMS.map((r) => <option key={r} value={r} style={{ background: C.ink }}>{r}</option>)}
      </select>
      ...
      <button onClick={submitPost} disabled={!ready || busy} style={{ fontWeight: 700, background: ready ? `linear-gradient(135deg, ${C.magenta}, ${C.violet})` : C.panel2, color: ready ? C.ink : C.mutedDim, borderRadius: 999, padding: "11px 24px", fontSize: 14, border: "none", cursor: ready && !busy ? "pointer" : "not-allowed" }}>{busy ? "Posting…" : "Post"}</button>
    </div>
  );
}
